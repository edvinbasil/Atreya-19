...
try {
    ...
    $from = $_POST['email']; // this is the sender's Email address
    $first_name = $from;
    $subject = "Subscription Box";
    $message = $from . " has just subscribed to your newsletter";
    ...
    $mail->send();
    echo 'Message has been sent <br> <a href="http://atreya.istenitc.org>Take me back to atreya</a>';
    $myfile = fopen("subscriptions.text", "a") or die("Unable to open file!");
    $txt = $from . "\n";
    fwrite($myfile, $txt);
    fclose($myfile);
} catch (Exception $e) {
